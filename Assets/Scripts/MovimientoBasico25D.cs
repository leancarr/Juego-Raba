...
public class MovimientoBasico25D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (estaAturdido)
        {
            ...
            else
            {
                ...
                return; // Bloquea los inputs de movimiento, salto y caída rápida
            }
        }
    ...
    void FixedUpdate()
    {
        // Si el jugador está aturdido, dejamos actuar libremente al AddForce del empuje
    ...
    // Función pública para que el Rapero pueda llamar e impactar a este jugador
    public void AplicarEmpujeYStun(Vector3 fuerzaEmpuje, float duracionStun)
    {
        estaAturdido = true;
        tiempoFinStun = Time.time + duracionStun;
        
        // Reseteamos velocidad actual y aplicamos el impacto físico puro de la onda
